...
# =============================================================================
# 2017 AUTHORITATIVE MATCHUP IDENTITY
# =============================================================================

AUTHORITATIVE_2017_MATCHUPS = pd.read_csv(
    Path("data/2017/matchups.csv")
)
...
for col in [
    ...
    if col in AUTHORITATIVE_2017_MATCHUPS.columns:
        AUTHORITATIVE_2017_MATCHUPS[col] = pd.to_numeric(
            AUTHORITATIVE_2017_MATCHUPS[col],
            errors="coerce",
        )


def is_recap_team_name(name):
...
def score_equal(a, b, tolerance=0.011):
    try:
...
def reconcile_2017_matchup(
    ...
    week_games = AUTHORITATIVE_2017_MATCHUPS[
        AUTHORITATIVE_2017_MATCHUPS["week"] == int(week)
    ].copy()
    ...
    for _, game in week_games.iterrows():
        ...
        # Same orientation.
        if (
            score_equal(scraped_left_score, game["score_1"])
            and score_equal(scraped_right_score, game["score_2"])
        ):
            candidates.append(
                (
                    str(game["team_1"]),
        ...
        if (
            score_equal(scraped_left_score, game["score_2"])
            and score_equal(scraped_right_score, game["score_1"])
        ):
            candidates.append(
                (
                    str(game["team_2"]),
    ...
    if not candidates:
        raise RuntimeError(
            f"No authoritative Week {week} matchup matches "
            f"{scraped_left_score} - {scraped_right_score}"
        )

    # Prefer candidates that agree with any real Yahoo team name
    # that survived the recap-label bug.
    narrowed = []

    for candidate in candidates:
        left_team, right_team, _, _ = candidate

        left_ok = (
            is_recap_team_name(scraped_left_team)
            or str(scraped_left_team).strip() == left_team
        )

        right_ok = (
            is_recap_team_name(scraped_right_team)
            or str(scraped_right_team).strip() == right_team
        )
    ...
    if len(candidates) != 1:
        raise RuntimeError(
            f"Ambiguous Week {week} score match for "
            f"{scraped_left_score} - {scraped_right_score}: "
            f"{candidates}"
        )

    true_left, true_right, left_score, right_score = candidates[0]

    if (
        str(scraped_left_team).strip() != true_left
        or str(scraped_right_team).strip() != true_right
    ):
        print(
            "Reconciled Yahoo labels: "
            f"{scraped_left_team} vs {scraped_right_team} "
            f"-> {true_left} vs {true_right}"
        )
            ...
            for team_id in range(1, 13):
                ...
                try:
                    (
                        ...
                        left_score,
                        right_score,
                    ) = reconcile_2017_matchup(
                        week=week,
                        scraped_left_team=left_team,
                        scraped_right_team=right_team,
                        scraped_left_score=left_score,
                        scraped_right_score=right_score,
                    )

                except Exception as exc:
                    print(
                        "Could not reconcile this Yahoo matchup "
                        f"against data/2017/matchups.csv: {exc}"
                    )
                    continue
