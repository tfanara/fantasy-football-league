...
SCRIPT_VERSION = "2018-clean-matchups-v5-filter-total"
...
def main():
    ...
    # Yahoo includes TOTAL summary rows in this rescued matchup.
    # They are not players and must not be counted as starters.
    if "lineup_slot" in rescue.columns:
        rescue = rescue[
            ~(
                rescue["lineup_slot"]
                .astype(str)
    ...
    print(
        f"Rescue player rows after removing TOTAL rows: {len(rescue)}"
    )
